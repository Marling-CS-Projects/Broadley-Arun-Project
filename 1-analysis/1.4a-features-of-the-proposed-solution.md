...
1. The timespan of my graph will be limited to 5 years and it wont up to the user to choose a longer or shorter timespan due to very complicated user interface skills that will be needed to implement this.
...
My linear regression analysis models will have a couple limits; one being the amount of historical data that will be fed into the linear regression analysis to produce a value. This should be limited for performance reasons as to much data will cause slow speeds and a worse user experience with little gain. I am limiting my parameters to: day of the week, season, volume traded, number of news article and insider trading. I chose these parameters as they seem like the ones that as individuals can bring the most gain for the users, also getting information for these parameters is a lot more possible then others.
