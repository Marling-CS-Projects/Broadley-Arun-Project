...
This box will allow the user to type in a company name or a stock ticker code (LLOY) for instance and then allow the user to select a stock from a drop-down list to make it easy for them to select a company.
...
1. The timespan of my graph will be limited to 5 years and it won't be updateable by the user to choose a longer or shorter timespan due to simplify the user interface.
...
1. R-Squared - How close the selected parameter fits the Growth parameter we ant to use. Answer ranges from 0 : No Fit, to 1: fits exactly.
2. P-Value - Larger values show less fit lower values show more fit.
3. F & F-DN describes the shape of the bestfit line
...
My linear regression analysis models will have a couple limits; one being the amount of historical data that will be fed into the linear regression analysis to produce a value. This should be limited for performance reasons as too much data will cause poor performance and a worse user experience with little gain. I am limiting my parameters to: day of the week, season, volume traded, number of news article and insider trading. I chose these parameters as they seem like the ones that as individuals can bring the most gain for the users, also getting information for these parameters is a lot more possible then others.

### Limitation 2:

running the linear regression comparisions, it would be good to run them against different output growth sets, for instance the following weeks growth, or the growth the next day. To reduce complexity, though, I will only run the comparisons against the following week's growth. &#x20;

### Specific Feature 2 - AI Calculations

Not sure about this yet. I'm still trying to work it out.
